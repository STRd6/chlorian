...
module.exports = function MidiFile (data) {
  ...
  for (var i = 0; i < header.trackCount; i++) {
    ...
    var state = {
      lastEventTypeByte: undefined
    }
    while (!trackStream.eof()) {
      var event = readEvent(trackStream, false, state)
      tracks[i].push(event)
    // console.log(event)
    }
  }
...
function readEvent (stream, skipTime, state) {
  var event = {}
  if (!skipTime) {
  ...
  if ((eventTypeByte & 0xf0) == 0xf0) {
    ...
    if ((eventTypeByte & 0x80) == 0) {
      ...
      */
      ...
      eventTypeByte = state.lastEventTypeByte
    } else {
      param1 = stream.readInt8()
      state.lastEventTypeByte = eventTypeByte
    }
